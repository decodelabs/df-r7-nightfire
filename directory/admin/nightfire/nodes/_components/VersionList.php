    ...
    public function addActionsField($list) {
        $list->addField('actions', function($version) {
            ...
            return [
                ...
                // Copy
                $this->html->link(
                        $this->uri->request('~admin/nightfire/nodes/edit?node='.$node->getId().'&version='.$version->getId(), true), 
                        $this->_('Copy')
                    )
                    ->setIcon('clipboard')
                    ->setDisposition('positive'),
                ...
                // Edit
                $this->html->link(
                        $this->uri->request('~admin/nightfire/nodes/edit-version?node='.$node->getId().'&version='.$version->getId(), true), 
                        $this->_('Edit')
                    )
                    ->setIcon('edit'),
                ...
                // Delete
                $this->html->link(
                        $this->uri->request('~admin/nightfire/nodes/delete-version?node='.$node->getId().'&version='.$version->getId(), true), 
                        $this->_('Delete')
                    )
                    ->setIcon('delete')
                    ->isDisabled($isActive)
