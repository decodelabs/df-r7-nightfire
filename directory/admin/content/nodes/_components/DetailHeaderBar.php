...
class DetailHeaderBar extends arch\component\template\HeaderBar {
    ...
    protected function _addOperativeLinks($menu) {
        $menu->addLinks(
            // Preview
            $this->import->component('NodeLink', '~admin/content/nodes/', $this->_record, $this->_('Preview'))
                ->setAction('preview')
            ...
            // Edit
            $this->import->component('NodeLink', '~admin/content/nodes/', $this->_record, $this->_('Edit node'))
                ->setAction('edit'),

            // Delete
            $this->import->component('NodeLink', '~admin/content/nodes/', $this->_record, $this->_('Delete node'))
                ->setAction('delete')
                ->setRedirectTo('~admin/content/nodes/')
        );
    }
    ...
    protected function _addSectionLinks($menu) {
        $menu->addLinks(
            // Details
            $this->import->component('NodeLink', '~admin/content/nodes/', $this->_record, $this->_('Details'), true)
                ->setAction('details')
                ->setIcon('details')
        );
        ...
        if($this->_record->getType() instanceof fire\type\IVersionedType) {
            $menu->addLinks(
                // Versions
                $this->import->component('NodeLink', '~admin/content/nodes/', $this->_record, $this->_('Versions'), true)
                    ->setAction('versions')
                    ->setIcon('list')
