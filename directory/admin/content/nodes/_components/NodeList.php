...
// Slug
    public function addSlugField($list) {
        $list->addField('slug', function($node) {
            return $this->import->component('NodeLink', '~admin/content/nodes/', $node)
    ...
    public function addActionsField($list) {
        $list->addField('actions', function($node) {
            return [
                ...
                // Edit
                $this->import->component('NodeLink', '~admin/content/nodes/', $node, $this->_('Edit'))
                    ->setAction('edit'),

                // Delete
                $this->import->component('NodeLink', '~admin/content/nodes/', $node, $this->_('Delete'))
                    ->setAction('delete')
            ];
        });
    }
